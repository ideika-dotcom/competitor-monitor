...
async def parse_demo(request: ParseDemoRequest):
    """
    Парсинг и анализ сайта конкурента через Selenium Chrome
    """
    logger.info("=" * 50)
    ...
    try:
        ...
        title, meta_description, h1, body_text, screenshot_bytes, error = await parser_service.parse_url(request.url)
        ...
        logger.info(f"  📌 Body text: {len(body_text) if body_text else 0} символов")
        ...
        parsed_content = ParsedContent(
            ...
            first_paragraph=body_text[:300] if body_text else None,
            analysis=analysis
        )
